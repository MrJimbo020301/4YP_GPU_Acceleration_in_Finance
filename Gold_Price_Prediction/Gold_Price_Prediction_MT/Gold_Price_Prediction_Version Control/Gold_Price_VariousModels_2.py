...
def train_and_evaluate(model_name):
    ...
    # Collect predictions for plotting over the entire dataset
    y_all_pred = []
    all_loader = DataLoader(TensorDataset(torch.from_numpy(X).float()), batch_size=batch_size, shuffle=False)
    model.eval()
    with torch.no_grad():
        for X_batch in all_loader:
            X_batch = X_batch[0].to(device)
            y_pred_batch = model(X_batch)
            y_all_pred.append(y_pred_batch.cpu().numpy())
    y_all_pred = np.concatenate(y_all_pred, axis=0)
    
    # Inverse scaling
    y_actual_inv = inverse_transform(y)
    y_pred_inv = inverse_transform(y_all_pred)
    
    # Store results
    results = {
        'model_name': model_name,
        'dates': dates,
        'y_actual_inv': y_actual_inv,
...
# Plotting the full dataset with continuous predicted curves
plt.figure(figsize=(14, 7))

# Actual Price
plt.plot(dates, inverse_transform(y), color='black', label='Actual Price')

# Plot predicted data for each model over the entire dataset
...
plt.title('Model Comparison on Gold Price Prediction with Continuous Predictions')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.grid(True)
...
# Zoomed-in plot for validation and test data
plt.figure(figsize=(14, 7))

# Focus on validation and test periods
zoom_mask = dates >= val_dates.min()
zoom_dates = dates[zoom_mask]
zoom_actual = inverse_transform(y[zoom_mask])
...
for model_name, results in all_results.items():
    plt.plot(
        zoom_dates,
        results['y_pred_inv'][zoom_mask],
        color=model_colors[model_name],
        linestyle='-',
        label=f'{model_name} Prediction'
    )

plt.title('Zoomed-in Model Comparison on Gold Price Prediction (Validation and Test Data)')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.grid(True)
