...
# Define the simplified LSTM model with increased dropout
class LSTMModel(nn.Module):
    def __init__(self):
        super(LSTMModel, self).__init__()
        self.lstm = nn.LSTM(input_size=len(features), hidden_size=32, batch_first=True)
...
optimizer = optim.Adam(model.parameters(), lr=0.001)  # Adjusted optimizer and learning rate
...
early_stopping = EarlyStopping(patience=20, min_delta=5e-4)
num_epochs = 160
...
with torch.no_grad():
    for X_batch, _ in train_loader:
        ...
        y_train_pred.append(y_train_pred_batch.numpy())
...
with torch.no_grad():
    for X_batch, _ in val_loader:
        ...
        y_val_pred.append(y_val_pred_batch.numpy())
